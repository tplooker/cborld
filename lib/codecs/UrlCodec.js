...
const uuidRegex = /(urn:uuid):(.*)/g;
export class UrnUuidCodec {
  encode(value) {
    // break the base58 DID URL into segments
    const matches = Array.from(value.matchAll(uuidRegex));
    const match = matches[0];

    // encode the DID URL segments
    const encodedUrl = (urlHeaders.has(match[1])) ? urlHeaders.get(match[1]) :
      match[1];
    // FIXME: Assumes all UUIDs are lowercase, which will preserve the value
    //        but might destroy signatures. Do not compress unless the UUIDs
    //        are all lowercase?
    const encodedUuid = uuid.parse(match[2]);
  ...
  decode(value) {
    ...
    const header = urlHeaders.get(encodedHeader);
    const uuidValue = uuid.stringify(Uint8Array.from(encodedUuid));
